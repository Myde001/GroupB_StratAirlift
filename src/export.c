...
int export_text(struct PATH *my_paths, char *output_file){
    ...
    text_file = fopen(output_file, "w");

    /* fopen() return NULL if last operation was unsuccessful */
    if(text_file == NULL){
        /* File not created hence return -1 */
        printf("Unable to create text file: %s.\n",output_file);
        return -1;
    }
...
int export_xml(struct STATE *my_state, char *output_file){
    FILE *xml_file=fopen(output_file,"w");

    /* fopen() return NULL if last operation was unsuccessful */
    if(xml_file == NULL){
        /* File not created hence exit */
        printf("Unable to create xml file: %s.\n",output_file);
        return -1;
    }

    /*Root tag for the xml*/
    fprintf(xml_file,"<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n");
    fprintf(xml_file, "<coupledModel name=\"Locations\">\n");
    create_port_data(xml_file);

    /*child tag components data*/
    fprintf(xml_file, "    <components>\n");
    for(int air_count = 0; air_count<my_sizes->locations; air_count++){
        create_component_data(xml_file, my_state, air_count);
    }
    fprintf (xml_file, "    </components>\n");

    /*child tag connections with eic and eoc data*/
    fprintf(xml_file, "    <connections>\n");
    for(int air_count = 0; air_count<my_sizes->locations; air_count++){
        create_connection_eic_data(xml_file, my_state, air_count);
    }
    for(int air_count = 0; air_count<my_sizes->locations; air_count++){
        create_connection_eoc_data(xml_file, my_state, air_count);
    }
    fprintf(xml_file, "    </connections>\n");
    fprintf(xml_file, "</coupledModel>\n");
